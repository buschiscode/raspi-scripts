...
#LCD_D6 = 22 #GPIO21 = Pi pin 13 (Use 21 on Rev.1 Pis)
...
#
def InitIO(): #Sets GPIO pins to input & output, as required by LCD board
...
def CheckSwitches(): #Check status of all four switches on the LCD board
...
def PulseEnableLine(): #Pulse the LCD Enable line; used for clocking in data
...
def SendNibble(data): #sends upper 4 bits of data byte to LCD data pins D4-D7
...
def SendByte(data,charMode=False): #send one byte to LCD controller
	...
	PulseEnableLine() #pulse the enable line
def InitLCD(): #initialize the LCD controller & clear display
...
def ShowMessage(string): #Send string of characters to display at current cursor position
	for character in string:
		SendChar(character)
def GotoLine(row): #Moves cursor to the given row
...
InitIO()
InitLCD()
ShowMessage('2222_Press a switch!')
while (True):
	GotoLine(1)
